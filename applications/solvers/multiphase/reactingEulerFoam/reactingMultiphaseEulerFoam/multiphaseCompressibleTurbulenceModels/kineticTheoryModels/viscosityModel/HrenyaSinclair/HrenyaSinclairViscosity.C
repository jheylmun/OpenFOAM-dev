...
) const
{
    ...
    (
        scalar(1) + da/(6.0*sqrt(2.0)*(phase + scalar(1.0e-5)))/L_
    );

    return da*sqrt(Theta)*
    (
        (4.0/5.0)*sqr(phase)*g0*(1.0 + e)/sqrtPi
      + (1.0/15.0)*sqrtPi*g0*(1.0 + e)*(3.0*e - 1)*sqr(phase)/(3.0-e)
      + (1.0/6.0)*sqrtPi*phase*(0.5*lamda + 0.25*(3.0*e - 1.0))
       /(0.5*(3.0 - e)*lamda)

...
#include "ChaoFrictionalStress.H"
...
) const
{
    ...
    return
       ...
       /max(alphap, phase.residualAlpha());
}
...
) const
{
    ...
    (
      ...
      + pow(max(alphap - alphaMinFriction_, scalar(0)), eta_)
       *(
            p_
           /(
                max(alphaMax - alphap, alphaDeltaMin_)
               *max(alphap, phase.residualAlpha())
            )
          - 1.0/max(sqr(phase), phase.residualAlpha())
        )
    )/pow(max(alphaMax - alphap, alphaDeltaMin_), p_);
}
...
) const
{
    ...
    forAll(D, celli)
    {
        if (alphap[celli] > alphaMinFriction_.value())
        {
            nuf[celli] =

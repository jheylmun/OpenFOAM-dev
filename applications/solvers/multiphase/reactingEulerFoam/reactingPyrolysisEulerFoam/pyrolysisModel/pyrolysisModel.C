...
:
    ...
    (
        refCast<solidReactionThermo>(fluid.phases()[solidPhaseName].thermo())
    ),
    ...
    solidThermo_
    (
        refCast<SolidThermo>(fluid.phases()[solidPhaseName]).thermo()
    ),
    gasThermo_
    (
        refCast<GasThermo>(fluid.phases()[gasPhaseName]).thermo()
    ),
    gasIndexes_(pyrolisisGases_.size(), -1),
    ...
    cellCounter_(0)
{
    ...
    label pGasI = 0;
    forAll(gasThermos_, gasI)
    {
        if (pyrolisisGases_.found(gasThermo_.species()[gasI])
        {
            gasIndexes_[pGasI] = gasI;
            pGasI++;
        }
    }
    ...
    forAll(gasThermos_, gasI)
    {
        ...
        (
            gasI,
            gasThermo_.speciesData()[gasIndexes_[gasI]]
        );
    }

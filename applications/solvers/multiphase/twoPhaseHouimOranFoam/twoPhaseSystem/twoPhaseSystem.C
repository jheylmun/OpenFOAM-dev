...
void Foam::twoPhaseSystem::relaxPressure()
{
    ...
    label nItt = 0;
//     if (instantRelaxation_)
//     {
//         // Equilibration due to drag
//         rhoThermo& thermo1(phase1_->thermo());
//         rhoThermo& thermo2(phase2_->thermo());
//         volScalarField& he1(thermo1.he());
//         volScalarField& he2(thermo2.he());
//
// //         volVectorField Ui("Ui",U());
// //
// //         he1 += sign(mag(Ui) - mag(phase1_->U()))*0.5*magSqr(Ui - phase1_->U());
// //         he2 += sign(mag(Ui) - mag(phase2_->U()))*0.5*magSqr(Ui - phase2_->U());
// //
// //         phase1_->U() = Ui;
// //         phase2_->U() = Ui;
//
//         // Equilibration due to pressure
//
...
//         )/rho();
//
//         volScalarField rho10(rho1);
//         volScalarField rho20(rho2);
//         volScalarField he10(he1);
//         volScalarField he20(he2);
//         volScalarField Pi0(Pi);
//         volScalarField dHEdp1("dHEdp1", thermo1.dHEdp());
//         volScalarField dHEdrho1("dHEdrho1", thermo1.dHEdrho());
//         volScalarField dHEdp2("dHEdp2", thermo2.dHEdp());
//         volScalarField dHEdrho2("dHEdrho2", thermo2.dHEdrho());
//
//         Pi =
//         (
//             phase1_->alphaRho()*phase1_->p()
...
//
//                 F[0] =
//                     2.0*rho1[celli]*rho10[celli]*(he1[celli] - he10[celli])
//                   + (Pi[celli] + Pi0[celli])*(rho1[celli] - rho10[celli]);
//
//                 F[1] =
//                     2.0*rho2[celli]*rho20[celli]*(he2[celli] - he20[celli])
//                   + (Pi[celli] + Pi0[celli])*(rho2[celli] - rho20[celli]);
//
//                 F[2] =
//                 (
//                     alphaRho1[celli]/rho1[celli]
//                   + alphaRho2[celli]/rho2[celli]
//                 ) - 1.0;
//
//                 D(0,0) =
//                     2.0*rho10[celli]*(he1[celli] - he10[celli])
//                   + 2.0*rho1[celli]*rho10[celli]*dHEdrho1[celli]
//                   + (Pi[celli] + Pi0[celli]);
//                 D(0,2) =
//                     2.0*rho1[celli]*rho10[celli]*dHEdp1[celli]
//                   + rho1[celli] - rho10[celli];
//
//                 D(1,1) =
//                     2.0*rho20[celli]*(he2[celli] - he20[celli])
//                   + 2.0*rho2[celli]*rho20[celli]*dHEdrho2[celli]
//                   + (Pi[celli] + Pi0[celli]);
//                 D(1,2) =
//                     2.0*rho2[celli]*rho20[celli]*dHEdp2[celli]
//                   + rho2[celli] - rho20[celli];
//
//                 D(2,0) = -alphaRho1[celli]/sqr(rho1[celli]);
//                 D(2,1) = -alphaRho2[celli]/sqr(rho2[celli]);
//
//                 vector dX = D.inv() & (F);
//
//                 rho1[celli] -= dX[0];
...
// //                 Info<<"dPi: " << dX[2]<<endl;
//
//                 error += sqr(dX[0]/rho1[celli]);
//                 error += sqr(dX[1]/rho2[celli]);
//                 error += sqr(dX[2]/Pi[celli]);
//            }
//             thermo1.heRho();
//             thermo2.heRho();
//
//             dHEdp1 = thermo1.dHEdp();
//             dHEdrho1 = thermo1.dHEdrho();
//             dHEdp2 = thermo2.dHEdp();
//             dHEdrho2 = thermo2.dHEdrho();
//
//             error = sqrt(error/(3*rho1.size()));
...
//
//         phase1_->p() = Pi;
//         alpha1 = alphaRho1/rho1;
//         phase1_->rho() = thermo1.rho();
//         phase1_->E() = he1 + 0.5*magSqr(phase1_->U());
//
//         phase2_->p() = Pi;
//         alpha2 = 1.0 - alpha2;
//         phase2_->rho() = thermo2.rho();
//         phase2_->E() = he2 + 0.5*magSqr(phase2_->U());
//
//         encode();
//     }
//     else
